...
typedef struct rtlsdr_struct {
	int blocksize;							// Read size for each buffer
	uint32_t id;							// SDR ID
	uint8_t *buffer[2];						// Buffers for two consecutive reads
	rtlsdr_dev_t *dev;						// SDR Device interface for use in functions
	pthread_t collection_t, initialize_t;	// Thread IDs for threading
} rtlsdr_struct;
struct rtlsdr_struct super;					// Struct for Supervisory RTL-SDR
struct rtlsdr_struct sdrs[NUM_SDRS];		// Structs for each RTL-STDR for data collection

typedef struct thread_struct {
	int id;				// SDR ID
	int freq;			// Frequency for collection and file_save
	rtlsdr_dev_t *dev;	// SDR Device interface for use in functions
} thread_struct;
